...
def chance_perf_distortion(result, distortion_id, save=False):
    ...
    plt.legend()
    if save:
        save_dir = Path(ROOT_DIR, REL_PATHS['analysis'], REL_PATHS['distortion_scan'])
        if not save_dir.is_dir():
            Path.mkdir(save_dir)
        title = f'{str(result)}.png'
...
if __name__ == '__main__':
    ...
    _distortion_id = 'noise'
    _identifier = 'n_scan_v3'
    ...
    chance_perf_distortion(_result, _distortion_id, save=True)
