...
from src.d04_analysis.results import load_result


class PlacesModelDistortionPerformanceResult(object):

    def __init__(self, result_id, convert_to_std=True):

        self.dataset, self.result, self.identifier = load_result(result_id)
        self.labels, self.predicts = self.result['image_labels'], self.result['predicts']
        self.labels = np.asarray(self.labels)
        self.predicts = np.asarray(self.predicts)
        self.res, self.blur, self.noise = self.dataset[:, 0], self.dataset[:, 1], self.dataset[:, 2]
...
#
#         model_distortion_performance = PlacesModelDistortionPerformanceResult(run, result_id, identifier=identifier)
#
#     return model_distortion_performance, output_dir


def get_places_distortion_performance_result(result_id):

    model_distortion_performance = PlacesModelDistortionPerformanceResult(result_id)
    output_dir = Path(ROOT_DIR, result_id)
    if not output_dir.is_dir():
        Path.mkdir(output_dir)
...
if __name__ == '__main__':
    ...
    _result_id = 'full-range'
    _model_distortion_performance, _output_dir = get_places_distortion_performance_result(_result_id)
